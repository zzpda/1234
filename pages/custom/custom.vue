<template>
	<view class="container">
		...
		<view class="search-box">
			...
			<view><image class="img-box" @click="showAddCustom" src="../../static/images/custom/add.png" mode=""></image></view>
		</view>
		<!-- 字母栏 -->
		...
		<unicloud-db v-slot:default="{ data, loading, error, options }" collection="bus_customers">
			...
			<view v-else>
				...
				<!-- 图片栏 -->
					<view class="fistT-box" @click="selectCustom(item)">{{ item.firstZ }}</view>
					<!-- 文字栏 -->
					<view class="info-box">
						<view class="customName">{{ item.c_name }}</view>
						<view class="info-other">
							<view class="info-left">
								<view class="c_phone1">电话:{{ item.c_phone1 }}</view>
								<view class="customComp">单位:{{ item.c_company }}</view>
							</view>
							<view class="info-right" v-show="false"><view class="productPice">价格:</view></view>
						</view>
					</view>
					<view><image class="img-edit" src="../../static/images/custom/edit.png" @click="handleEdit(item)"></image></view>
					<view><image class="img-delete" src="../../static/images/custom/del.png" @click="handleDelete(item)"></image></view>
				</view>
			</view>
		</unicloud-db>
	</view>
...
//https://www.jianshu.com/p/1dac53aed886 //uni-app做A-Z排序通讯录、索引列表
